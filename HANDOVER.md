...
合計 配送 670,657 件 / 寄附 510,849 件。残り 2 自治体は順次追加予定。
（大垣市は 2026-05-29 に追加。件数はドライラン検証値で、社内PCでの取込後に確定。取込スクリプト: `tests/ingest_ogaki.py`）
...
| **寄付金額・件数** | `donations` |
| **謝礼品価格** | `shipments` |
| **返礼率** | 謝礼品価格(shipments) / 寄付金額(donations) |
