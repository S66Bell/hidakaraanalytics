...
| 粗利 | （概念ごと削除） | — | 表示しない |
| 粗利率 | **返礼率** | `expense_ratio` | `謝礼品価格 / 寄付金額` |

新規 UI・レポート・ドキュメントを作成する際は必ずこの用語で統一。SQL クエリ内部の英語フィールド名（`revenue`, `total_cost`, `expense_ratio`）は内部実装のためそのままで OK。
...
│       ├── channel.py          ← 🛒 チャネル別（サイト×カテゴリ / サイト×事業者 クロス）
│       ├── municipality.py     ← 🏙️ 自治体詳細（単一自治体ドリルダウン + 前年比）
